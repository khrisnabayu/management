            ...
            <div class="row gy-4 posts-list">
                <div class="col-lg-6">
                    <article class="d-flex flex-column">
                        ...
                        <div class="col-lg-6">
                        <h2 class="title">
                            <a href="{{ route('profilecompany.blogdetail', $villa->id) }}">{{ $villa->name}}</a>
                        </h2>
                        </div>
                        </div>
                ...
                <div class="col-lg-6">
                    <article class="d-flex flex-column">
                        ...
                        <div class="row">
                            <div class="col-lg-12">
                                <h2 class="title">
                                    <a href="#">Booking Confirmation</a>
                                </h2>
                            </div>

                            <div class="col-lg-12 mt-4">
                                <form action="{{ route('profilecompany.submitbookingreservation') }}" method="POST" enctype="multipart/form-data">
                                    @csrf
                                    <div class="row">
                                        <div class="col-md-6 form-group">
                                          <i style="color:#1D5D9B;" class="bi bi-check-circle-fill"></i> <label class="small mb-1" for="currentTitle"> In : {{$dataCheckIn}}</label>
                                          <input type="hidden"  id="inputCheckIn" name="check_in" class="form-control"  placeholder="Check In" value='{{$dataCheckIn}}'>
                                        </div>
                                        <div class="col-md-6 form-group mt-3 mt-md-0">
                                          <i style="color:#1D5D9B;" class="bi bi-check-circle-fill"></i> <label class="small mb-1" for="currentTitle"> Out : {{ $dataCheckOut}}</label>
                                          <input type="hidden"  id="inputCheckOut" name="check_out" class="form-control"  placeholder="Check Out" value='{{$dataCheckOut}}'>
                                        </div>
                                    </div>
                                    <div class="form-group mt-3">
                                        <input type="hidden"  id="inputVillaType" name="villa_type" class="form-control"  placeholder="Villa Type" value='{{$villa->id}}'>
                                    </div>
                                    ...
                                    <div class="form-group mt-3">
                                        <input type="number"  id="inputPhonenumber" name="phonenumber" class="form-control" placeholder="Phonenumber" required>
                                    </div>
                                    <div class="my-3">
                                    </div>
                                    <p style="font-size:12px;"><b>{{ $termcondition->name }} :</b></p>
                                    <p style="font-size:12px;">{{ $termcondition->description }}</p>

                                    <div class="read-more mt-auto align-self-end">
                                        <button type="submit" class="btn" style="font-size: 16px; color: white; background: #54B435; padding: 8px 23px; border-radius: 4px;">Confirmation Booked</button>
                                    </div>
                                </form>
                            </div>
                        </div>
